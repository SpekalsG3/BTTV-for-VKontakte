...
bttvForVKNS.createElement = (tag, attrs = {}, children = []) => {
...
bttvForVKNS.hasClassName = (node, className) => {
  return node?.nodeType === bttvForVKNS.NodeTypes.ELEMENT_NODE && [...node.classList].includes(className)
}

bttvForVKNS.replaceEmotesAtImMess = (imMess) => {
  if (!bttvForVKNS.hasClassName(imMess, 'im-mess')) {
    return
  }

  const plainTextNode = imMess.children[2]
  if (plainTextNode && [...plainTextNode.classList].includes('im-mess--text')) {
    bttvForVKNS.replaceEmotes(plainTextNode)
  }
}

bttvForVKNS.replaceEmotesAtImStack = (stack) => {
  if (!bttvForVKNS.hasClassName(stack, 'im-mess-stack')) {
    return
  }

  for (const imMess of stack.children[1].children[1].children) {
    bttvForVKNS.replaceEmotesAtImMess(imMess)
  }
}

bttvForVKNS.isCorrectMutation = ({ mutation, target, validateAddedNodes }) => {
  return bttvForVKNS.hasClassName(mutation?.target, target) && mutation.addedNodes.length > 0 && validateAddedNodes(mutation.addedNodes)
}

bttvForVKNS.getEmoteImgUrl = function (emote) {
  return `https://cdn.betterttv.net/${(emote.frankerfacez ? "frankerfacez_emote" : "emote")}/${emote.id}/${emote.size}`
}

bttvForVKNS.toImg = function(name) {
  var emote = bttvForVKNS.bttvEmotes[name];
  var imgWrapper = bttvForVKNS.createElement("div", {
    className: `bttv_img-wrapper${(emote.modification !== undefined ? " " + emote.modification : "")}`
  }, [
    bttvForVKNS.createElement("img", {
      src: bttvForVKNS.getEmoteImgUrl(emote),
...
bttvForVKNS.replaceEmotes = function(node) {
  ...
  for (var s of node.childNodes) {
    if (s.nodeType === bttvForVKNS.NodeTypes.TEXT_NODE) {
...
bttvForVKNS.handleScrollBar = function(value, bybar) {
  var flag = false;
...
bttvForVKNS.insertInEditable = function(name, toReplace) {
  let currentNodeText;
...
bttvForVKNS.predictErrorMsg = bttvForVKNS.createElement("div", {
  className: "bttv_predcited-error--inner",
  innerHTML: "No such emote"
});

bttvForVKNS.predictedMenu = bttvForVKNS.createElement("ul", {
  className: "bttv_predicted-menu"
});
...
bttvForVKNS.predictEmote = function(e) {
  if (e.code == "Tab") {
    ...
    if (bttvForVKNS.predictedEmotes.length == 0) {
      ...
      bttvForVKNS.lastTabbedEmote = (bttvForVKNS.lastTabbedEmote + 1) % bttvForVKNS.predictedEmotes.length
    }
    ...
    if (e.key == ' ') {
      ...
      if (wordBeforeTabbed == "") {
        ...
        bttvForVKNS.predictedEmotes = Object.keys(bttvForVKNS.bttvEmotes).filter(emote => {
          return emote.toLowerCase().startsWith(wordBeforeTabbed.toLowerCase()) &&
      ...
      if (bttvForVKNS.predictedEmotes.length == 0) {
        ...
        for (const e of bttvForVKNS.predictedEmotes) {
          const emote = bttvForVKNS.bttvEmotes[e];
          const item = bttvForVKNS.createElement("li", {
...
bttvForVKNS.emoteScrollable.addEventListener('onwheel' in document.createElement('div') ? 'wheel' : 'mousewheel', function(e) {
  bttvForVKNS.handleScrollBar(bttvForVKNS.menuScrollOffset+e.deltaY, false);
});
...
bttvForVKNS.fillEmotesMenu = function() {
  ...
  for (const name in bttvForVKNS.bttvEmotes) {
    const emote = bttvForVKNS.bttvEmotes[name];

    if (!bttvForVKNS.settings.showGifs && emote.animated) {
      continue;
    }

    const emotePreview = bttvForVKNS.createElement("div", {
      title: name,
    ...
    emotePreview.addEventListener("click", function() {
      bttvForVKNS.insertInEditable(this.title, false);
    });
...
bttvForVKNS.emoteButtonWrapper.addEventListener("click", function() {
  if (bttvForVKNS.emotesMenuShown) {
    ...
    const emotesMenuCoors = bttvForVKNS.emoteButtonWrapper.getBoundingClientRect();
...
bttvForVKNS.appendEmoteMenu = function() {
  const inputButtons = document.getElementsByClassName("im_chat-input--buttons")[0];
...
window.addEventListener("bttvForVKSettingsChange", function(e) {
  ...
  const createCheckbox = function(name, desc, inputName) {
  ...
  const popupCloseBtn = bttvForVKNS.createElement("div", {
  ...
  for (const box of bttvForVKNS.checkboxes) {
    const input = box.lastElementChild.firstElementChild.firstElementChild;
  ...
  for (const tab of bttvForVKNS.tabs)
  ...
  const vkMenu = document.getElementById("top_profile_menu");
  const settingsBtn = bttvForVKNS.createElement("div", {
  ...
  bttvForVKNS.pageBodyObserver?.disconnect()
  bttvForVKNS.pageBodyObserver = new (window.MutationObserver || window.WebKitMutationObserver)(function(muts) {
    for (const mutation of muts) {
      try {
        if (bttvForVKNS.isCorrectMutation({
          mutation: mutation,
          target: 'ui_clean_list',
          validateAddedNodes: (nodes) => nodes.length === 1 && bttvForVKNS.hasClassName(nodes[0], 'im-mess')
        })) {
          // Sent new message
          bttvForVKNS.replaceEmotesAtImMess(mutation.addedNodes[0])
        ...
        if (bttvForVKNS.isCorrectMutation({
          mutation: mutation,
          target: '_im_peer_history',
          validateAddedNodes: (nodes) => nodes.length === 1 && bttvForVKNS.hasClassName(nodes[0], 'im-mess-stack')
        })) {
          // Sent message with reply
          bttvForVKNS.replaceEmotesAtImStack(mutation.addedNodes[0])
        ...
        if (bttvForVKNS.isCorrectMutation({
          mutation: mutation,
          target: '_im_peer_history',
          validateAddedNodes: (nodes) => bttvForVKNS.hasClassName(nodes[nodes.length - 1], 'im-mess-stack')
        })) {
          // Opened dialog
          for (const stack of mutation.addedNodes) {
            bttvForVKNS.replaceEmotesAtImStack(stack)
          }
        ...
        if (bttvForVKNS.isCorrectMutation({
          mutation: mutation,
          target: 'im-replied-container--content',
          validateAddedNodes: (nodes) => nodes.length === 1 && bttvForVKNS.hasClassName(nodes[0], 'im-replied')
        })) {
          // Opened dialog with cached reply message
          // Selected msg for reply
          bttvForVKNS.replaceEmotes(mutation.addedNodes[0].children[1].children[1])
        ...
        if (bttvForVKNS.isCorrectMutation({
          mutation: mutation,
          target: 'nim-dialog--preview',
          validateAddedNodes: (nodes) => nodes.length === 1 && nodes[0].nodeType === bttvForVKNS.NodeTypes.TEXT_NODE,
        })) {
          // Dialog preview change
          bttvForVKNS.replaceEmotes(mutation.target)

          continue
        }

        // Changing pages
        // Emotes in preview for forwared messages
        // Opening dialog directly
      } catch (e) {
        console.log('[fixObserver] ERROR -', e)
      }
    }
  })
  bttvForVKNS.pageBodyObserver.observe(document.getElementById("page_body"), {
    subtree: true,
    attributes: false,
    childList: true
  });

  bttvForVKNS.boxLayerObserver?.disconnect()
  bttvForVKNS.boxLayerObserver = new (window.MutationObserver || window.WebKitMutationObserver)(function (muts) {
    for (const mutation of muts) {
      try {
        if (bttvForVKNS.isCorrectMutation({
          mutation: mutation,
          target: 'box_body',
          validateAddedNodes: (nodes) => nodes.length === 1 && (
            bttvForVKNS.hasClassName(nodes[0], 'im_single_log_wrap') || bttvForVKNS.hasClassName(nodes[0], 'im-important-box')
          )
        })) {
          // Forwarded tab
          // Starred tab
          for (const el of muts[1].addedNodes[0].getElementsByClassName("im-mess--text wall_module")) {
        ...
        console.log('[boxLayerObserver] ERROR -', e)
      }
    }
  });
