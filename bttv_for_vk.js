...
bttvForVKNS.parseEmotes = function(text) {
  ...
  for (var i = 0; i <= l; i++) {
    if (text[i] == ' ' || i == l) {
      if (word in bttvForVKNS.bttv_emotes) {
        if (textBefore != "")
...
bttvForVKNS.predictedEmotes = Object.keys(bttvForVKNS.bttv_emotes);
...
bttvForVKNS.predictEmote = function(e) {
  if (e.code == "Tab") {
    ...
    if (!bttvForVKNS.lastPressedTab) {
      ...
      if (wordBeforeTabbed == "")
        return;
      bttvForVKNS.predictedEmotes = Object.keys(bttvForVKNS.bttv_emotes).filter(emote => emote.toLowerCase().startsWith(wordBeforeTabbed.toLowerCase()));
    ...
    if (e.key == ' ')
      ...
      if (bttvForVKNS.predictedEmotes.length == 0)
        ...
        for (var e of bttvForVKNS.predictedEmotes) {
          var emote = bttvForVKNS.bttv_emotes[e];
...
bttvForVKNS.emoteButtonWrapper.addEventListener("click", function() {
  if (bttvForVKNS.emotesMenuShown) {
    ...
    bttvForVKNS.emoteButtonWrapper.style.webkitFilter = "grayscale(100%)";
    bttvForVKNS.emoteButtonWrapper.style.filter = "grayscale(100%)";
  } else {
    ...
    bttvForVKNS.emoteButtonWrapper.style.webkitFilter = "none";
    bttvForVKNS.emoteButtonWrapper.style.filter = "none";
  }
  bttvForVKNS.emotesMenuShown = !bttvForVKNS.emotesMenuShown;
});

function bttv_visualizeEmotesOnPage() {
  // All messages
